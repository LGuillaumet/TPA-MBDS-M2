...
marketing = read.csv("Marketing.csv", header=T, sep=",", dec='.')
str(marketing)
...
marketingGrouped = marketing %>% group_by(carCategory) %>% summarise(age = age, sexe=sexe, taux=taux, situationFamiliale = situationFamiliale, nbEnfantsAcharge=nbEnfantsAcharge, X2eme.voiture=X2eme.voiture, carCategoryName=carCategoryName);marketingGrouped
