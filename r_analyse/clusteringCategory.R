...
#Utiliser les fichiers catalogue.csv et immatriculation.csv
immat = read.csv(file.choose(), header=T, sep=",", dec='.', stringsAsFactors = T)
catag = read.csv(file.choose(), header=T, sep=",", dec='.', stringsAsFactors = T)
...
#Grouper par cluster
dataGrouped = data %>% group_by(cluster) %>% summarise(puissance = puissance, longueur=longueur, nbPlaces=nbPlaces, nbPortes=nbPortes);dataGrouped
View(dataGrouped)
